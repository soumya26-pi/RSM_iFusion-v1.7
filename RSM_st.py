...
#     bbox=(pt['x0'],ending_keys['top'],pt['x1'],ending_keys['bottom'])
...
#     bbox=(header['x0'],header['top'],pt['x1'],ending_keys['bottom']+2)
    bbox=(header['x0'],header['top'],pt['x1'],ending_keys['bottom']+2)
    bbox_kv=(pt['x0']-12,ending_keys['top'],pt['x1']+5,ending_keys['bottom']+3.8)
#     bbox=(pt['x0'],ending_keys['top'],pt['x1'],ending_keys['bottom']+2)
#     words_list_filtered=[word for word in words_list if word['x0']>=bbox[0] and word['x1']<=bbox[2] and word['top']>=bbox[1] and word['bottom']<=bbox[3]]
    return header,ending_keys,pt,bbox_kv,bbox,

def extract_kv(header,ending_key,ending_column):
    page_kv = []   
    for i in page_cnt:
        global text
        text = pdf_ob.pages[i].extract_words(x_tolerance=3, y_tolerance=3, keep_blank_chars=True, use_text_flow=False, horizontal_ltr=True, vertical_ttb=True, extra_attrs=[])
        global pno
        pno = i+1
        ...
        if out == None:
            ...
            box2_kv = out[-2]
            image =pdf_ob.pages[i].within_bbox(box2_kv, relative=False)
            te = image.extract_text()
#             print('Value output:------->',te)
            
            if te == '':
                image1 =pdf_ob.pages[i].crop(box2_kv, relative = False)
                te = image1.extract_text()
#                 print('if te is NA value:------->', te)
            nex = [header, ending_key,ending_column,te,str(i+1)]
            page_kv.append(nex)
...
#         ss['Table_ending_column'] = ss['Table_ending_column'].apply(
...
#             page_2 = pdf_ob.pages[pgno].extract_words(extra_attrs=['y0', 'y1'],use_text_flow=True,keep_blank_chars=True)
...
#             # words = new_words
#             # st.write("page_number",i)
                        
#             words = [i for i in page_2 if len(i['text'].strip()) > 1]
#             final_words = []
#             new_words = []
            
#             obj_word = handle_multipleline(words)
#             obj_word2 = hamdle_multiplelinerow(obj_word)
#             obj_word3 = handle_multiple_row2(obj_word2)
                    
#             for word in obj_word3:
#                 word_x = word['text'].lower()
#                 if word_x.isalpha() and len(set(word_x)) == 1:
#                     continue
#                 else:
...
#                  'Page Number']]
#             final_out['Final Values'] = final_out['Final Values'].str.split(r"[A-Za-z]+").str[0]
...
def run_extraction(file_name, reference_name):
    ...
    if len(reference[reference['PDF Key'].notnull()])>=1:
        ...
        reference.drop_duplicates(inplace=True)
        # reference['PDF Table Header Org'] = reference['PDF Table Header']
        reference['PDF Table Header'] = reference['PDF Table Header'].apply(lambda x: x.replace('\n(', '(').replace(' (', '(') if type(x) == str else x)
        # reference['PDF Key Org'] = reference['PDF Key']
        reference['PDF Key'] = reference['PDF Key'].apply(lambda x: x.replace(' (', '(') if type(x) == str else x)
        reference = reference[['PDF Table Header','PDF Key', 'columns_to_extract']].applymap(str.lower)

        reference.drop_duplicates(inplace=True)
        # df['PDF Table Header'] = df['PDF Table Header'].apply(lambda x: x.lower().replace('\n', ' ') if type(x) == str else x)
        # print('After cleansing \n', df)
        rowss1 = reference.values
        # print(rowss1)
        kv = []
        dataframe_kv = []
        for i in range(len(rowss1)):
            h, endk,endc = rowss1[i]
            kv.append(extract_kv(h, endk,endc))
        for i in kv:
            for j in i:
                dataframe_kv.append(map(lambda x: x.replace('\n', ' ').capitalize(), j))
            ...
            if result:
                ...
                if task == "Deselect":
                    ...
                    if p_name:
                        ...
                        if saver is not None:
                            ...
                            os.makedirs("./csvdump" ,exist_ok=True)
                ...
                if tasknew == "Deselect":
                    ...
                    if pp:
                        ...
                        st.write(ld)
                        st.header("RSM")
                        os.makedirs('./pdfdump' ,exist_ok=True)
